...
Analytics provide the analysis on the use pattern of the users running the game app along, from selecting the target appropriated for your purpose.<br/>

To use the analysis data that Analytics offer, you need to, first, register your company and app, followed by applying the SDK provided by Analytics.(SDK supports iOS, Android and Unity).

# Service Structure

Analytics SDK collects the app users’ log and exposes the set in Analytics web via your app. The analysis/promotion system in Analytics collects and analyzes the log from SDK and communicates with the game server to make the possible.
...
- Admin: Admin may inquire the analytics of concerned app and change information on operation such as app setting/info. 
...
##### Set Group
This setting is required for creating user group. You may categorize groups depending on the level, accumulated purchasing amount, number of visits, number of non-visited days, number of friends and so forth. Set group bracket is changeable; however the change will not be applied to the past days.
